...
def find():
    ...
    try:
        ...
        ##### cursor = cursor.sort( [('student_id', pymongo.DESCENDING), ('score', pymongo.ASCENDING)] )
    ...
    for doc in cursor:
        ...
        print "prev_stu :", prev_stu
        print_dict(doc)

        if prev_stu != doc['student_id']:
            print "student different", doc['_id']
            grades.delete_one( { '_id' : doc['_id'] } )

        prev_stu = doc['student_id']
